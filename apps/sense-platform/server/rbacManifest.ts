...
export const rbacManifest = [
  ...
  { procedure: "commerce.checkout.createOrder", requirement: "authenticated", ownership: "ctx.user.id owns cart and resulting order" },
  { procedure: "commerce.checkout.mine", requirement: "authenticated", ownership: "ctx.user.id" },
  { procedure: "commerce.checkout.byId", requirement: "authenticated", ownership: "ctx.user.id owns order" },
  { procedure: "commerce.checkout.adminHandoffs", requirement: "role_or_ownership", roles: ["admin"] },
  { procedure: "commerce.checkout.updateInventory", requirement: "role_or_ownership", roles: ["admin"] },
  { procedure: "commerce.checkout.receiveStatusEvent", requirement: "role_or_ownership", roles: ["admin"] },
  ...
  { procedure: "ai.assistOperations", requirement: "role_or_ownership", roles: ["staff", "field", "supervisor", "admin"], ownership: "sanitized report text; no mutation" },
  { procedure: "ai.assistTripPlanning", requirement: "authenticated", ownership: "sanitized access preferences and published stop data; no mutation" },
